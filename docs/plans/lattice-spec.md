...
| `LATTICE_TRUSTED_CIDRS` | `127.0.0.0/8,::1/128,100.64.0.0/10` | Comma-separated CIDR ranges treated as trusted when `LATTICE_AUTH_DISABLED=true`. Default is loopback + Tailscale CGNAT only — RFC1918 LAN ranges are deliberately *not* in the default and must be opted into explicitly. Both IPv4 and IPv6 CIDRs are supported (matching uses `ipaddr.js`). Malformed entries fail at boot. Untrusted source IPs receive a 401 and one warning log per IP. |
